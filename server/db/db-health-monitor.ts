...
export class EmpireDatabaseHealthMonitor {
  ...
  /**
   * Safe wrapper for optimized table health check - Billion-Dollar Performance
   */
  private async checkTableHealthOptimizedSafe(tableName: string): Promise<TableHealthMetrics> {
    try {
      return await this.checkTableHealthOptimized(tableName);
    } catch (error) {
      return {
        tableName,
        rowCount: 0,
